...
internal class ShortcutButton : DWButton
{
    ...
    public ShortcutButton(UIObject? parent, string shortcutSaveId, Vec2 position, Vec2 size, UIAlignment alignment = UIAlignment.TopCenter) : base(parent, position, size - 12.5f, null, alignment)
    {
        ...
        {
            savedShortcut = (ShortcutSave)SaveManager.GetOrDefault(shortcutSaveId);
        }
        else
        {
        ...
        shortcutTitle.Font = Resources.FileResources.InterBold;
        shortcutTitle.Color = Theme.TextSecond;
        shortcutTitle.SilentSetActive(false);
        AddLocalObject(shortcutTitle);

        shortcutIcon = new DWImage(this, Resources.FileResources.FileIcon, new Vec2(0, 0), new Vec2(20, 20), UIAlignment.MiddleLeft);
    ...
    internal void SetShortcut(ShortcutSave save)
    {
        if(!string.IsNullOrEmpty(save.path))
        {
            ...
            SaveManager.AddOrUpdate("shortcuts." + saveId, JsonConvert.SerializeObject(save));
            SaveManager.SaveAll();
    ...
    void LoadShortcut()
    {
        ...
        var shortcut = (ShortcutSave)JsonConvert.DeserializeObject<ShortcutSave>((string)SaveManager.GetOrDefault("shortcuts." + saveId));
