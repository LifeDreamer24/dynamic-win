...
class RegisterWeatherWidgetSettings : IRegisterableSetting
{
    ...
    public void LoadSettings()
    {
        if (SaveManager.Contains(SettingID))
            saveData = JsonConvert.DeserializeObject<WeatherWidgetSaveData>((string)SaveManager.GetOrDefault(SettingID));
        else
        {
...
public class WeatherWidget : WidgetBase
{
    ...
    {
        AddLocalObject(new DWImage(this, Resources.FileResources.Location, new Vec2(20, 17.5f), new Vec2(12.5f, 12.5f), UIAlignment.TopLeft)
        {
        ...
        AddLocalObject(new DWImage(this, Resources.FileResources.Weather, new Vec2(20, 37.5f), new Vec2(12.5f, 12.5f), UIAlignment.TopLeft)
        {
        ...
        weatherTypeIcon = new DWImage(this, Resources.FileResources.Weather, new Vec2(0, 0), new Vec2(100, 100), UIAlignment.MiddleRight)
        {
            Color = Theme.TextThird,
            allowIconThemeColor = true
        };
    ...
    void UpdateIcon(string weather)
    {
        if (weather.ToLower().Contains("sun") || weather.ToLower().Contains("clear"))
            weatherTypeIcon.Image = Resources.FileResources.Sunny;
        else if (weather.ToLower().Contains("cloud") || weather.ToLower().Contains("overcast"))
            weatherTypeIcon.Image = Resources.FileResources.Cloudy;
        else if (weather.ToLower().Contains("rain") || weather.ToLower().Contains("shower"))
            weatherTypeIcon.Image = Resources.FileResources.Rainy;
        else if (weather.ToLower().Contains("thunder"))
            weatherTypeIcon.Image = Resources.FileResources.Thunderstorm;
        else if (weather.ToLower().Contains("snow"))
            weatherTypeIcon.Image = Resources.FileResources.Snowy;
            ...
            weatherTypeIcon.Image = Resources.FileResources.Foggy;
        else if (weather.ToLower().Contains("windy") || weather.ToLower().Contains("breezy"))
            weatherTypeIcon.Image = Resources.FileResources.Windy;
        else
