...
public class AudioVisualizer : UIObject
{
    ...
    public AudioVisualizer(UIObject? parent, Vec2 position, Vec2 size, UIAlignment alignment = UIAlignment.TopCenter, int length = 16, int averageAmpsSize = 0, Color Primary = null, Color Secondary = null) : base(parent, position, size, alignment)
    {
    ...
    {
        Color pColor = Utils.Color.Lerp(Secondary, Primary, averageAmplitude * 2);
        return pColor;
    }

    public Color GetInverseActionCol()
    {
        Color pColor = Utils.Color.Lerp(Primary, Secondary, averageAmplitude * 2);
        return pColor;
    }
        ...
        lock (fftLock)
        {
            ...
            if (!avAmpsMode)
            {
                ...
                for (int i = 0; i < barHeight.Length / divisor; i++)
                {
                    ...
                    Color pColor = Utils.Color.Lerp(Secondary, Primary, Mathf.Clamp(barHeight[i], 0, 1));
            ...
            else
            {
                ...
                for (int i = 0; i < smoothAverageAmps.Length / divisor; i++)
                {
                    ...
                    Color pColor = Utils.Color.Lerp(Secondary, Primary, Mathf.Clamp(smoothAverageAmps[i], 0, 1));
                    paint.Color = GetColor(pColor).Value();
